...
async function HandleSubmit(e){
  try{
    e.preventDefault()
    let inputs = document.querySelectorAll('input[type="text"],input[type="date"],input[type="tel"],input[type="email"],textarea');
   ...
   Array.from(inputs).map((item) => {
    ...
    const response = await axios.post("https://backend-learn-kmt8.onrender.com/user/register", details) 
